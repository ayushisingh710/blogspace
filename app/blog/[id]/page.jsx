...
const BlogDetails = ({ params }) => {
  ...
  return (
    <section className="container max-w-3xl">
        ...
        <Link href={`/user/${blogDetails?.authorId?._id.toString()}`}>
          <div className="flex flex-col justify-center items-center py-10">
            ...
            <div className="text-center">
              <p className="text-primary">{blogDetails?.authorId?.name}</p>
              <p>{blogDetails?.authorId?.designation}</p>
            </div>
          </div>
        </Link>

        <div className="text-center space-y-3 text-primary">
          <h2>{blogDetails?.title}</h2>

          <p>{blogDetails?.excerpt}...</p>

          <p className="flex items-center justify-center gap-3">
            <span className="text-primary">{blogDetails?.category}</span>
                    ...
                    <div key={pIndex}>
                      {pIndex ===
                        Math.floor(
                          ...
                          <blockquote className="border-l-10 border-primary  border-spacing-8 italic mb-5">
                            <p className="ml-5 text-primary pb-5 text-2xl">{blogDetails?.quote}</p>
                          </blockquote>
                        )}
      ...
      <div className="py-12">
        <div className="flex gap-10 items-center text-xl justify-center py-6">
      ...
      <div>
        {!session?.user && (
          <h3 className="text-red-700">Kindly login to leave a comment.</h3>
        )}
